        ...
        try:
            ...
            while True:
                ...
                if jalan == '':
                    ...
                    # Exit registrasi -> tutup koneksi & kembali
                    next()
                    cursor.close()
                    connection.close()
                    return 
            ...
            if connection:
                connection.close()
            continue
        finally: # pastikan koneksi ditutup
            if cursor:
                try:
                    cursor.close()
                except:
                    pass  # Kalau sudah tertutup, abaikan
            if connection:
                try:
                    connection.close()
                except:
...
def menu_pembeli(id_pengguna, username):
    while True: # agar tidak memanggil menu pembeli secara recursive
...
def menu_penjual(id_pengguna, username):
    ...
    | |\/| |/ _ \ '_ \| | | | | |_) / _ \ '_ \ | | | | |/ _` | |
    | |  | |  __/ | | | |_| | |  __/  __/ | | || | |_| | (_| | |
    |_|  |_|\___|_| |_|\__,_| |_|   \___|_| |_||_|_|\__,_|\__,_|_|
                                            |__/               
