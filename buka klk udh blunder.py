...
def penjual_kelola_produk(id_pengguna, nama_pengguna):
    ...
    try:
        ...
        if pilihan == "1":
            query = "SELECT id_produk, nama_produk, harga_produk, stok_produk, kategori_produk FROM produk WHERE is_delete = FALSE"
            cursor.execute(query)
...
def penjual_riwayat_penjualan(id_pengguna, nama_pengguna):
    ...
    try:
        ...
        # Ambil produk yang dijual oleh penjual ini
        query = """
        ...
        if riwayat:
            ...
            print(f"\n💰 Total Penjualan (Selesai): Rp{total_penjualan:,}")
        else:
